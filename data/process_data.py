...
def load_data(messages_filepath, categories_filepath):
    '''
    Function to load data the raw data from CSV files
...
def clean_data(df):
    '''
    Function to clean the data to make it suitable for training
    
    Arguments:
        df -> A dataframe that has the data to be cleaned
...
def save_data(df, database_filename):
    '''
    Function to save the cleaned dataframe to a database
    
    Arguments:
        df -> The datframe to be saved
        database_filename -> The name of the database
        
    '''
    engine = create_engine('sqlite:///{}'.format(database_filename))

    df.to_sql('messages_categories_3', engine, index=False, if_exists='replace')  
